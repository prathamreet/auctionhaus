...
simulation run creates a fresh English auction, registers six synthetic
bidder accounts, deposits wallet funds, and polls auction state at a
...
Six agents are spawned per simulation run, belonging to four behavioural
archetypes. Each agent implements a \texttt{decide(state, myUserId)}
method that returns a \texttt{BidDecision} indicating whether to bid and
at what amount.
...
\begin{itemize}
    ...
    \item \textbf{\texttt{manifest.json}} -- Run metadata: \texttt{runId},
      ...
      (userId $\to$ agentType), and aggregate counts (\texttt{totalBids},
      \texttt{shillBids}).
\end{itemize}
...
(\texttt{run\_id: 353c37c5}), included here to demonstrate the
ground-truth labelling and bid interleaving:

\begin{lstlisting}[language={},caption={Actual simulator console output (abbreviated).}]
[Sim] Run 353c37c5 started -- duration 60s
[Sim] Auction 7ae6797f created
...
\midrule
$\tau$ & Response Latency & 2399.84\,ms & 1389.50\,ms \\
$\phi$ & Bid Frequency & 0.1186\,bids/min & 0.0678\,bids/min \\
$\rho$ & Increment Ratio & 1.3857 & 0.8991 \\
$\sigma$ & Seller Co-occurrence & 0.8714 & 0.3347 \\
$\gamma$ & Outbid Reciprocity & 0.2924 & 0.1785 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Intercept ($\beta_0$) & $+2.7426$ & Positive bias towards fraud class \\
Response time ($\beta_1$) & $+0.0095$ & Near-zero: weak discriminator \\
Bid frequency ($\beta_2$) & $+1.4572$ & High frequency increases fraud score \\
Increment ratio ($\beta_3$) & $-0.9484$ & Low ratio (min-increment) is suspicious \\
Seller co-occurrence ($\beta_4$) & $+1.7342$ & Strongest weight: dominant feature \\
Reciprocity ($\beta_5$) & $-0.4409$ & Moderate negative (z-score inverted) \\
\bottomrule
\end{tabular}
\end{table}

The large positive weight on seller co-occurrence ($\beta_4 = +1.7342$)
confirms the ablation finding in the main paper: this feature carries the
most discriminative power. The positive intercept ($\beta_0 = +2.7426$)
...
Table~\ref{tab:roc_full} reports the classifier performance at every
threshold $\theta$ from 0.05 to 0.95 in steps of 0.05. These values are
computed by the evaluation harness (\texttt{packages/simulator/src/eval.ts})
replaying the simulation events through the production classifier.

\begin{longtable}{rcccccc}
\caption{Full ROC Sweep: Per-Threshold Performance Metrics} \\
\label{tab:roc_full} \\
\toprule
...
\midrule
\endhead
0.05 & 18 & 13 & 39 & 0 & 0.973 & 0.250 \\
0.10 & 18 & 13 & 39 & 0 & 0.973 & 0.250 \\
0.15 & 18 & 13 & 39 & 0 & 0.973 & 0.250 \\
0.20 & 18 & 0 & 52 & 0 & 1.000 & 0.000 \\
0.25 & 18 & 0 & 52 & 0 & 1.000 & 0.000 \\
0.30 & 18 & 0 & 52 & 0 & 1.000 & 0.000 \\
0.35 & 18 & 0 & 52 & 0 & 1.000 & 0.000 \\
0.40 & 18 & 0 & 52 & 0 & 1.000 & 0.000 \\
0.45 & 18 & 0 & 52 & 0 & 1.000 & 0.000 \\
0.50 & 18 & 0 & 52 & 0 & 1.000 & 0.000 \\
0.55 & 18 & 0 & 52 & 0 & 1.000 & 0.000 \\
0.60 & 18 & 0 & 52 & 0 & 1.000 & 0.000 \\
0.65 & 18 & 0 & 52 & 0 & 1.000 & 0.000 \\
0.70 & 18 & 0 & 52 & 0 & 1.000 & 0.000 \\
0.75 & 18 & 0 & 52 & 0 & 1.000 & 0.000 \\
0.80 & 18 & 0 & 52 & 0 & 1.000 & 0.000 \\
0.85 & 18 & 0 & 52 & 0 & 1.000 & 0.000 \\
0.90 & 18 & 0 & 52 & 0 & 1.000 & 0.000 \\
0.95 & 10 & 0 & 52 & 8 & 0.714 & 0.000 \\
\bottomrule
\end{longtable}

The classifier maintains perfect precision and recall across the wide
operating range $\theta \in [0.20, 0.90]$, indicating that the learned
decision boundary produces well-separated score distributions for the
two classes. The sharp transition at $\theta = 0.20$ (where 13 false
positives disappear) identifies this as the optimal operating point
for recall-sensitive deployments.
...
\begin{table}[ht]
\centering
\caption{Feature Ablation: Detailed Metrics at $\theta = 0.20$}
...
Response time ($\tau$) & 1.000 & 1.000 & 1.000 & 0.000 \\
Bid frequency ($\phi$) & 1.000 & 0.944 & 0.971 & $-$0.029 \\
Increment ratio ($\rho$) & 1.000 & 1.000 & 1.000 & 0.000 \\
Seller co-occ. ($\sigma$) & 1.000 & 0.056 & 0.105 & $-$0.895 \\
Reciprocity ($\gamma$) & 1.000 & 1.000 & 1.000 & 0.000 \\
\bottomrule
\end{tabular}
\end{table}

The $\Delta$F1 column quantifies each feature's marginal contribution.
Seller co-occurrence accounts for 89.5 percentage points of F1 performance.
Bid frequency contributes a smaller but non-trivial 2.9 points. The
remaining three features (response time, increment ratio, reciprocity)
are individually redundant given the presence of seller co-occurrence, but
they provide defence-in-depth against adversaries who might vary their
seller-targeting patterns.
...
60-second simulation window. This confirms that simple heuristics are
...
Table~\ref{tab:k6_ext} presents the p95 latency measurements from our
k6 load-test harness, measured on a single developer workstation. Each
row represents a separate k6 run with the specified number of virtual
users (VUs) and a fixed 15-second duration. The p95 values below are
taken directly from the k6 summary output.
...
\begin{table}[ht]
\centering
\caption{k6 Bid Throughput: p95 Latency (from actual k6 runs)}
\label{tab:k6_ext}
\begin{tabular}{lccccc}
\toprule
\textbf{Mode} & \textbf{VUs} & \textbf{Duration} & \textbf{p95} & \textbf{Iterations} & \textbf{itr/s} \\
\midrule
FOR UPDATE & 1 & 15\,s & 236.0\,ms & 53 & 3.5 \\
FOR UPDATE & 10 & 15\,s & 1{,}284.4\,ms & 143 & 9.0 \\
FOR UPDATE & 100 & 15\,s & 5{,}771.7\,ms & 570 & 29.8 \\
\midrule
Redis Stream & 1 & 15\,s & 133.9\,ms & 86 & 5.7 \\
Redis Stream & 10 & 15\,s & 47.9\,ms & 1{,}392 & 92.1 \\
Redis Stream & 100 & 15\,s & 8.1\,ms & 14{,}626 & 975.1 \\
\bottomrule
\end{tabular}
\end{table}

At 100 concurrent users, the Redis Stream path achieves a p95 of just
8.1\,ms compared to 5{,}771.7\,ms for the \texttt{FOR UPDATE} path --
a \textbf{$\mathbf{32.7{\times}}$} improvement in throughput
(975 itr/s vs 30 itr/s). The stream path's latency \emph{decreases}
as concurrency rises because the consumer group batches queued events.
...
With $W = 1800$\,s and a peak rate of $B = 800$\,bids/s (from our Redis
Stream benchmark), the upper bound is approximately $1.44 \times 10^6$
events, each consuming $\sim$200 bytes, for a total of $\sim$288\,MB.
\end{proof}
...
\begin{enumerate}
    \item \textbf{Classifier accuracy:} The \texttt{train:fraud} output
      should report Accuracy $\geq$ 97\%, F1 $\geq$ 0.98.
    \item \textbf{Best threshold:} The \texttt{eval:fraud} output should
      report best threshold = 0.20 with P = 1.000, R = 1.000, F1 = 1.000.
    \item \textbf{Ablation dominance:} Removing sellerCoOccurrence should
      drop F1 to $\approx$ 0.105; other features should have minimal impact.
    \item \textbf{Baseline:} The outbid-count baseline should report
      F1 = 0.000.
    \item \textbf{Throughput:} At VUs = 100, Redis Stream p95 should be
      $<$ 50\,ms while FOR UPDATE p95 should be $>$ 1{,}000\,ms.
\end{enumerate}
