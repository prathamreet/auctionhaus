...
1. **A1. Replace Float with Decimal.** Migrate Prisma schema money fields to `Decimal @db.Decimal(18,2)`. Add a `lib/decimal.ts` helper. Update all services. Generate migration. *Acceptance: `grep -r "Float" back/prisma/schema.prisma` returns nothing on money fields.*
2. **A2. Add row locks.** Introduce `prisma.$queryRaw\`SELECT ... FOR UPDATE\`` in `placeBid`, `withdraw`, `endAuction`, `buyNow`. *Acceptance: a k6 script firing 50 concurrent bids cannot produce two bids with the same `currentPrice` (verified by querying `SELECT amount, count(*) FROM bids GROUP BY auctionId, amount HAVING count(*) > 1`).*
3. **A3. Add indexes.** Migration adds B-tree on `Auction(status)`, `Auction(endTime)`, `Auction(type, status)`, `Bid(auctionId, status)`, `Bid(bidderId)`, `Notification(userId, isRead)`, `Transaction(userId, createdAt)`. Add GIN on `to_tsvector(title || ' ' || description)` and switch `getAuctions` search to FTS.
4. **A4. Fix sealed-bid privacy.** ~~Replace the broken `name: false` with an explicit DTO that omits bidder identity *and* randomizes order while live.~~ *(partial — 2026-05-28: `bid.service.getAuctionBids` rewritten to use `select`-based DTO that masks amount + bidder for non-owner bids and orders by `createdAt asc`, plus `bid.controller.placeBid` redacts the `bid:new` socket emit while sealed+active. **Remaining:** `auction.service.getAuctionById` still includes the last 20 bids with bidder names — fix in next pass.)*
