...
11. **RP3, RP5.3, RP6. Deferred while paper was frozen.** Now implemented in full — see Phase RP-2 below (the 2026-06-03 paper rejection lifted the freeze).

### Phase RP-2 — Honest Pipeline Rebuild (2026-06-03, after paper rejected on domain scope)
Goal: the first submission was rejected on domain scope, not technical merit. With the paper no longer frozen, implement the corrections that were deferred to avoid fabricating published numbers. The code becomes genuinely correct; the resubmission's numbers are then regenerated from it. **User runs `sim:run` ×N → `train:fraud` → `eval:fraud`; I cannot run servers.**

1. ~~**RP2-1. Shared dataset module.**~~ ✓ **2026-06-03:** `packages/simulator/src/dataset.ts` — single replay path used by both train and eval. `sellerCoOccurrence` grounded in the real auction owner (`manifest.auctionOwners`). Deterministic FNV-1a train/test split keyed on bidId (no leakage). Reads active runs from `runs/`, excludes `_`-prefixed archives.
2. ~~**RP2-2. Multi-auction simulator (RP3.1/3.2).**~~ ✓ **2026-06-03:** `run.ts` rewritten — primary seller lists `SIM_PRIMARY_AUCTIONS` (default 3) auctions + a decoy second seller lists one. Shill/collusion span all primary auctions (high σ); truthful/sniper take one each, truthful may also bid the decoy (low σ per seller, yet active). Per-(persona,auction) agent instances reuse one user/credential. `manifest.auctionOwners` records every auction's seller.
3. ~~**RP2-3. Trainer rewrite.**~~ ✓ **2026-06-03:** `train.ts` fits LR on the TRAIN partition only (mean/std computed on train, no test leakage) and writes `fraud.classifier.ts` directly. v1/v2 fiction + `--write/--confirm` guard removed. `train.v2.ts` deleted; `train:fraud:v2` scripts removed from both package.json files.
4. ~~**RP2-4. Eval rewrite (RP5.3 + RP6).**~~ ✓ **2026-06-03:** `eval.ts` reports on the HELD-OUT TEST partition — ROC sweep, mean-ablation (set feature to its mean → z=0), and two real baselines: outbid-count>10 and a best single-feature decision stump (fit on train, scored on test). Writes `paper/figures/{roc_data,ablation_data}.json` + `paper/tables/metrics.tex` on test numbers. No Prisma dependency anymore (reads minIncrement from the manifest).
5. ~~**RP2-6. Escrow settlement fix (audit Critical).**~~ ✓ **2026-06-03:** Closed the one audit finding that contradicted a paper claim ("atomic escrow settlement"). Auction-end now pays the seller automatically: English + Sealed settle in `workers/index.ts::endAuction` (winner `heldAmount` → seller via idempotent `settleWithinTx(WON_AUCTION)` in a fresh auction-locked tx + `PAYMENT_RECEIVED` notify); Dutch settles immediately in `bid.service.placeBid` on accept; buy-now unchanged (`DIRECT_SALE`). All idempotent via the `Settlement` row, so `confirmWinnerPayment` remains a harmless backstop and double-pay is impossible. `WinnerCertificate`'s "auto-settled / credited to your wallet" copy is now true (gated to winner/seller). `endAuction` is untested and the Dutch `placeBid` test still passes; 130 tests stay green.
6. **RP2-5. Regenerate + update paper. (USER ACTION)** Run the pipeline, then update the paper's simulator section, weights table, ablation, metrics table, and baseline discussion from the freshly generated artifacts. The threshold shipped by `train.ts` is 0.5 (LR boundary); `eval.ts` reports the full ROC so the operating point is a data-driven choice.

Old single-auction corpus retained under `back/packages/simulator/runs/_paper-snapshot/` for provenance (excluded from the pipeline). This supersedes the RP §5 Q1 "Option B" decision. Remaining audit items (`updateAuction` dead route, `/fraud/perf` diagnostics, dormant `BidSequencer`) reviewed and accepted as intentional, non-harmful surface.
