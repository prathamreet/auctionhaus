# Graph Report - auctionhaus  (2026-05-28)

## Corpus Check
- 135 files · ~71,881 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1119 nodes · 1437 edges · 135 communities (105 shown, 30 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 22 edges (avg confidence: 0.83)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `34db43f6`
...
6. `AuctionHaus — Complete System Documentation` - 16 edges
7. `done` - 16 edges
8. `SESSION CLOSED — Where We Are Leaving (read this first next session)` - 16 edges
9. `AuctionHaus — Complete System Documentation` - 16 edges
10. `done` - 16 edges
...
Cohesion: 0.05
Nodes (38): 1. Deep Core Architecture Analysis, 1. Reconnection Message Sequencer (State Recovery), 2. Critical Concurrency & Race Condition Audits (The "Wow" Factor for Teachers), 2. Shill Bidding & Collusion Graph Analytics (Academic Innovation), 3. Escalating Dynamic Dutch Auctions, 3. High-Integrity Financial Data Hardening, 4. Distributed Transaction Safety (Redis Redlock), 5. Major Project Feature Expansion (Beyond CRUD) (+30 more)

### Community 3 - "Auto-Bid & Queue Engine"
Cohesion: 0.09
Nodes (11): processAutoBids(), placeBid(), bid, getAuctionBids(), placeBid(), defaultAuction, dutchAuction, prevBid (+3 more)
...
Cohesion: 0.06
Nodes (30): Acceptance signals for the user to run, Acceptance signals for the user to run, Acceptance signals (run together with A1's after `prisma migrate dev`), Commit, Commit, Context Loaded, Continuation 2 — Phase A6 atomic auto-bid ladder (user said "resume"), Continuation — Phase A3 perf indexes (rolled into the same commit) (+22 more)

### Community 6 - "Financial Integrity Core"
Cohesion: 0.22
Nodes (19): buyNow(), cancelAuction(), createAuction(), getAuctionById(), getAuctions(), mockData, updateAuction(), setAutoBid() (+11 more)

### Community 7 - "User Profile Controllers"
Cohesion: 0.13
Nodes (19): deleteAllNotifications(), deleteNotification(), getNotifications(), markRead(), notifyUser(), consoleSpy, mockNotification, confirmWinnerPayment() (+11 more)

### Community 8 - "Frontend Architecture"
Cohesion: 0.1
Nodes (21): 4. Advanced Architectural Evolutions, code:block10 (┌──────────────┐      ┌─────────────┐      ┌──────────────┐), code:typescript (await biddingQueue.add('process-bid', { auctionId, bidderId,), code:typescript (// Synchronously cascading recursion inside placeBid's post-), code:typescript (await autoBidQueue.add('evaluate-autobids', { auctionId, cur), code:typescript (// Inside workers/index.ts), code:typescript (// Enqueue notification delivery asynchronously), code:typescript (const notificationWorker = new Worker('notifications', async) (+13 more)

### Community 9 - "Notification Controllers"
Cohesion: 0.13
Nodes (14): cancelAutoBid(), getMyAutoBid(), defaultAuction, dutchAuction, sealedAuction, secondAutoBid, topAutoBid, AsymmetricMatcher (+6 more)
...
Cohesion: 0.2
Nodes (8): router, router, authenticate(), AuthRequest, requireAdmin(), nextFunction, result, router
...
Cohesion: 0.21
Nodes (7): bullMQConnection, redis, redisPub, redisSub, auctionQueue, dutchAuctionQueue, notificationQueue
...
Cohesion: 0.31
Nodes (6): cancelAutoBid(), getMyAutoBid(), setAutoBid(), mockResult, nextFunction, router
...
## Knowledge Gaps
- **586 isolated node(s):** `fs`, `path`, `srcDir`, `ignorePatterns`, `files` (+581 more)
...
- **Why does `io` connect `User Profile Controllers` to `Frontend Pages & UI`, `Auto-Bid & Queue Engine`, `Real-Time Socket Layer`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `getSocket()` connect `Frontend Pages & UI` to `User Profile Controllers`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Why does `AuctionHaus Backend — Full Technical Deep Dive` connect `PostCSS Config` to `Product Requirements`, `Development Rules`, `Community 73`, `Community 74`, `Community 47`, `Community 48`, `Next.js Config`, `Community 57`, `Community 58`, `Backend ESLint`, `Request Lifecycle`, `Community 62`, `Community 63`?**
