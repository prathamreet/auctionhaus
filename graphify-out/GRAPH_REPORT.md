...
## Corpus Check
- 124 files · ~55,180 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 742 nodes · 991 edges · 94 communities (65 shown, 29 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 22 edges (avg confidence: 0.83)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `9403b7d1`
...
Cohesion: 0.07
Nodes (39): getAllAuctions(), getAllUsers(), getDashboardStats(), getFraudFlags(), moderateAuction(), suspendUser(), mockGroupBy, createAuction() (+31 more)

### Community 2 - "Backend Service Layer"
Cohesion: 0.06
Nodes (33): cancelAutoBid(), getMyAutoBid(), processAutoBids(), setAutoBid(), defaultAuction, dutchAuction, secondAutoBid, topAutoBid (+25 more)
...
Cohesion: 0.18
Nodes (9): router, router, router, router, authenticate(), requireAdmin(), nextFunction, router (+1 more)
...
Cohesion: 0.23
Nodes (8): initSocketGateway(), nextFn, app, bootstrap(), ExtendedRequest, httpServer, initWorkers(), consoleSpy
...
Cohesion: 0.2
Nodes (9): 15. Request Lifecycle (End to End), 1. Tech Stack & Why, 2. Project Entry Point, AuctionHaus Backend — Full Technical Deep Dive, code:block1 (1. Load environment variables (.env)), code:typescript (const app = express();), code:typescript (app.use((req, _res, next) => {), code:block48 (Browser (Alice)                 Server                      ) (+1 more)
...
Cohesion: 0.43
Nodes (5): cancelAutoBid(), getMyAutoBid(), setAutoBid(), mockResult, nextFunction
...
Cohesion: 0.67
Nodes (3): 12. Watchlist, code:typescript (@@unique([userId, auctionId])  // can't add same auction twi), code:typescript (prisma.watchlistItem.upsert({)
...
## Knowledge Gaps
- **346 isolated node(s):** `fs`, `path`, `srcDir`, `ignorePatterns`, `files` (+341 more)
...
- **Why does `io` connect `Backend Service Layer` to `Frontend Pages & UI`, `Auction Scheduler & Dutch`?**
  _High betweenness centrality (0.074) - this node is a cross-community bridge._
- **Why does `getSocket()` connect `Frontend Pages & UI` to `Backend Service Layer`?**
  _High betweenness centrality (0.073) - this node is a cross-community bridge._
- **Why does `createError()` connect `Admin & API Controllers` to `Backend Service Layer`, `Auction Create Form`?**
