# Graph Report - auctionhaus  (2026-05-30)

## Corpus Check
- 191 files · ~102,590 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1635 nodes · 2189 edges · 175 communities (140 shown, 35 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 27 edges (avg confidence: 0.83)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `665beee3`
...
Cohesion: 0.1
Nodes (22): bullMQConnection, redis, redisInvalidateSub, redisPub, redisSub, deleteAllNotifications(), deleteNotification(), getNotifications() (+14 more)
...
Cohesion: 0.06
Nodes (30): 0. North Star, 1.1 Stack & Topology, 1.2 Domain, 1.3 Real-Time Layer, 1.4 Background Jobs, 1.5 Frontend Shape, 1.6 What Already Works Well (Keep These), 1. What the Code Actually Is (Ground Truth) (+22 more)

### Community 8 - "Frontend Architecture"
Cohesion: 0.15
Nodes (17): cancelAutoBid(), processAutoBids(), setAutoBid(), placeBid(), placeBidStream(), bid, getAuctionBids(), placeBid() (+9 more)
...
Cohesion: 0.21
Nodes (18): buyNow(), cancelAuction(), createAuction(), getAuctionById(), getAuctions(), mockData, updateAuction(), serializeMoney() (+10 more)
...
Cohesion: 0.13
Nodes (8): AdminStats, Tab, AuctionRowData, BidRowData, disconnectSocket(), getSocket(), reconnectSocket(), Listener
...
Cohesion: 0.14
Nodes (14): getMyAutoBid(), defaultAuction, dutchAuction, sealedAuction, secondAutoBid, topAutoBid, nextFunction, AsymmetricMatcher (+6 more)

### Community 16 - "API Routes & Catalogue"
Cohesion: 0.15
Nodes (12): router, router, router, query, router, router, authenticate(), CachedUser (+4 more)

### Community 17 - "Frontend ESLint Config"
Cohesion: 0.13
Nodes (11): Bid, BidHistory(), CommitmentPanel(), AuctionForInfo, InfoPanel(), KeyDetails(), TYPE_INFO, Auction (+3 more)
...
Cohesion: 0.19
Nodes (10): router, initSocketGateway(), nextFn, rateLimiter, strictRateLimiter, app, bootstrap(), ExtendedRequest (+2 more)
...
Cohesion: 0.13
Nodes (14): 10. Dependencies, 11. Security Measures, 12. Test Accounts (from seed), 13. Known Issues (from checklist.md), 14. Data Flow: Bid Placement, 15. Data Flow: Auction End, 1. What This Codebase Does, 2. Folder Structure (+6 more)
...
Cohesion: 0.21
Nodes (9): AuctionForPrice, AutoBidPanel(), AutoBidState, BidForm(), DutchAccept(), PricePanel(), WinnerCertificate(), formatMoney() (+1 more)
...
Cohesion: 0.2
Nodes (7): dismissFlag(), AppError, errorHandler(), err, issues, nextFunction, zodError
...
Cohesion: 0.18
Nodes (5): inter, metadata, Theme, ThemeBootstrap(), ThemeToggle()
...
Cohesion: 0.24
Nodes (8): cancelAutoBid(), getMyAutoBid(), setAutoBid(), mockResult, nextFunction, body, { commitHash }, router
...
Cohesion: 0.2
Nodes (3): createAuction(), createAuctionSchema, auction

### Community 50 - "Community 50"
Cohesion: 0.36
Nodes (8): getAllAuctions(), getAllUsers(), getDashboardStats(), getFraudFlags(), moderateAuction(), suspendUser(), mockGroupBy, invalidateUser()

### Community 51 - "Community 51"
Cohesion: 0.31
Nodes (7): settleWithinTx(), toNum(), confirmWinnerPayment(), refundLosers(), defaultAuction, sellerWallet, winnerWallet
...
Cohesion: 0.27
Nodes (6): clearStored(), Commitment, loadStored(), saveStored(), storageKey(), StoredCommit
...
Cohesion: 0.2
Nodes (10): 4. Advanced Architectural Evolutions, code:block10 (┌──────────────┐      ┌─────────────┐      ┌──────────────┐), code:typescript (await biddingQueue.add('process-bid', { auctionId, bidderId,), code:typescript (import { createAdapter } from '@socket.io/redis-adapter';), Evolution 1: Centralized FIFO Bidding Queue (The Holy Grail of Scaling), Evolution 4: Horizontal Socket.io Scaling (Redis Adapter), The Problem, The Problem with Database Locks (+2 more)
...
Cohesion: 0.25
Nodes (4): AUCTION_LABEL, AUCTION_TONE, STATUS_TONE, Tone
...
Cohesion: 0.52
Nodes (5): generateToken(), getMe(), login(), register(), mockUser

### Community 87 - "Community 87"
Cohesion: 0.29
Nodes (3): AuthRequest, result, router
...
Cohesion: 0.29
Nodes (7): 2. Critical Concurrency & Race Condition Audits (The "Wow" Factor for Teachers), code:typescript (// back/src/modules/auctions/auction.service.ts), code:typescript (return prisma.$transaction(async (tx) => {), The Architectural Solution (Transactional State Validation), The Bug (Current Implementation), The Exploit, Vulnerability 1: The "Double-Claim" Race Condition in `buyNow`
...
Cohesion: 0.47
Nodes (5): commitBid(), getCommitments(), hashCommitment(), revealBid(), toCents()
...
Cohesion: 0.33
Nodes (6): code:typescript (const wallet = await prisma.wallet.findUnique({ where: { use), code:typescript (// Enforce positive balance directly in the database update ), The Architectural Solution (Transactional Balance Validation), The Bug (Current Implementation), The Exploit, Vulnerability 3: Read-Then-Write Race Condition in Wallet `withdraw`
...
Cohesion: 0.4
Nodes (5): code:typescript (// Synchronously cascading recursion inside placeBid's post-), code:typescript (await autoBidQueue.add('evaluate-autobids', { auctionId, cur), Evolution 2: Decoupled Auto-Bid Execution, The Problem, The Solution
...
## Knowledge Gaps
- **809 isolated node(s):** `fs`, `path`, `srcDir`, `ignorePatterns`, `files` (+804 more)
...
- **Why does `getSocket()` connect `Database Schema Design` to `Frontend Architecture`, `Frontend ESLint Config`, `Community 88`, `Financial Integrity Core`?**
  _High betweenness centrality (0.062) - this node is a cross-community bridge._
- **Why does `io` connect `Frontend Architecture` to `Design System Philosophy`, `Database Schema Design`, `Auction Create Form`?**
  _High betweenness centrality (0.062) - this node is a cross-community bridge._
- **Why does `createError()` connect `App Layout & Providers` to `Community 96`, `Git Convention`, `Auction Create Form`, `Frontend Architecture`, `Community 48`, `Community 49`, `Community 50`, `Community 51`, `Community 86`?**
